...
def test_plot_recursive_results_formatting(tmp_path, monkeypatch):
    res = {
        "ticker": "TST",
        "start": "2023-01-01",
        "end": "2023-01-03",
    ...
    # Figure size should match reference
    w, h = fig.get_size_inches()
    assert w == pytest.approx(70)
    assert h == pytest.approx(40)

    # No suptitle in reference
    assert fig._suptitle is None

    # Legend font sizes should be scaled consistently
    for ax in fig.axes:
        leg = ax.get_legend()
        if leg:
            for text in leg.get_texts():
                assert text.get_fontsize() == pytest.approx(16)
