...
async def backtest_options_sync_or_async(cfg: RecursionConfig) -> Dict[str, Any]:
    ...
    async with PolygonAPIClient(api_key=s.polygon_api_key) as client:
        ...
        while cur <= end_dt:
            ...
            expiries: List[datetime] = list_expiries(
                weekday=cfg.expiring_weekday,
                start_date=cur,
                end_date=cur,  # same day → we want "the next matching expiry on/after cur"
                expiring_wk=cfg.expiring_wks,
                trading_dates_df=hist[["date"]],  # compatible with your earlier signature
            )
