...
def check_portfolio_xml():
    try:
        ...
        url = base_url + "/v1/accounts/" + acc.account["accountIdKey"] + "/portfolio"
        # Explicitly ask for XML
        headers = {"Accept": "application/xml"}
        response = session.get(url, header_auth=True, headers=headers)
        
        if response.status_code == 200:
            print("\n" + "="*40)
            print("XML PORTFOLIO RESPONSE (Snippet)")
            print("="*40)
