...
def check_pm_endpoint():
    try:
        ...
        # Test the portfoliomargin endpoint
        url = base_url + "/v1/accounts/" + acc.account["accountIdKey"] + "/portfoliomargin.json"
        print(f"Testing URL: {url}")
        response = session.get(url, header_auth=True)
        
        if response.status_code == 200:
            print("SUCCESS! Found portfoliomargin endpoint.")
