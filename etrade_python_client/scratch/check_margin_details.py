...
def check_margin_details_endpoint():
    try:
        ...
        # Test the margindetails endpoint
        url = base_url + "/v1/accounts/" + acc.account["accountIdKey"] + "/margindetails.json"
        print(f"Testing URL: {url}")
        response = session.get(url, header_auth=True)
        
        if response.status_code == 200:
            print("SUCCESS! Found margindetails endpoint.")
