...
def check_balance_margin_sub_endpoint():
    try:
        ...
        # Test /balance/margin.json
        url = base_url + "/v1/accounts/" + acc.account["accountIdKey"] + "/balance/margin.json"
        print(f"Testing URL: {url}")
        response = session.get(url, header_auth=True)
        
        if response.status_code == 200:
            print("SUCCESS! Found /balance/margin.json.")
...
if __name__ == "__main__":
    check_balance_margin_sub_endpoint()
