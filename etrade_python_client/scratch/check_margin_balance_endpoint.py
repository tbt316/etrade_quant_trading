...
def check_margin_balance_endpoint():
    try:
        ...
        # Test the marginbalance endpoint
        url = base_url + "/v1/accounts/" + acc.account["accountIdKey"] + "/marginbalance.json"
        print(f"Testing URL: {url}")
        response = session.get(url, header_auth=True)
        
        if response.status_code == 200:
            print("SUCCESS! Found marginbalance endpoint.")
