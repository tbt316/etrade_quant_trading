...
def validate_dashboard_credentials(settings: dict[str, Any]) -> None:
    ...
    if (
        len(pin) < 8
        or len(pin) > 64
        or pin.lower() in {"00000000", "12345678", "password", "changeme"}
    ):
        raise RuntimeSafetyError("dashboard action PIN is weak")


def validate_dashboard_auth_secret(value: Any) -> None:
    """Require the canonical high-entropy format generated by this runtime."""
    ...
    if (
        ...
        raise RuntimeSafetyError(
            "dashboard authentication secret must be a generated 256-bit hex value"
        )
