...
def backtest_strategy(stock_data_input, window_size_ratio=10, sigma=3, baseline='MOV_AVG', option_strategy='CALL_ONLY', expire_period=30, metric_eval_period=180):
    ...
    if option_strategy == 'CALL_ONLY':
        ...
        breaches_band = np.where(trend == 'up', future_prices < signals['Down'], future_prices > signals['Upper'])
    ...
    if breaches_band.any():
        if option_strategy == 'CALL_ONLY':
            max_breach_price = future_prices[breaches_band].max() - signals['Upper']
            signals.loc[breaches_band, 'Signal'] = 1
            signals.loc[breaches_band, 'Loss'] = max_breach_price * 100
        elif option_strategy == 'PUT_ONLY':
            min_breach_price = future_prices[breaches_band].min() - signals['Down']
            signals.loc[breaches_band, 'Signal'] = 1
            signals.loc[breaches_band, 'Loss'] = -min_breach_price * 100
        elif option_strategy == 'CALL_AND_PUT':
            max_breach_price = (future_prices[breaches_band_up].max() - signals['Upper']) * breaches_band_up
            min_breach_price = (future_prices[breaches_band_down].min() - signals['Down']) * breaches_band_down
            signals.loc[breaches_band, 'Signal'] = 1
            signals.loc[breaches_band, 'Loss'] = (max_breach_price - min_breach_price) * 100
        elif option_strategy == 'CALL_OR_PUT':
            max_breach_price = (future_prices[breaches_band].max() - signals['Upper']) * (trend == 'down')
            min_breach_price = (future_prices[breaches_band].min() - signals['Down']) * (trend == 'up')
            signals.loc[breaches_band, 'Signal'] = 1
            signals.loc[breaches_band, 'Loss'] = (max_breach_price + min_breach_price) * 100
